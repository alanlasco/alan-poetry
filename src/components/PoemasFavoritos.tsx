...
export const PoemasFavoritos = () => {
  ...
  const getPoemaClass = (poemaId: number) => {
    return `${
      poemaStates[poemaId] ? "btn-toggle-poema-v" : "btn-toggled-poema-v"
    }`;
  };
  ...
  const renderPoemas = (poema: reqPoema) => {
    ...
    return (
      <div className="favouriteAnimation" key={poema.id_poema}>
        <div
          key={poema.id_poema}
          className={`poemas-container ${containerExpanded}`}
        >
          ...
          <p
            key={`fecha-${poema.id_poema}`}
            className={getPoemaClassFecha(poema.id_poema)}
          >
            {poema.fecha}
          </p>
          <button
  ...
  const currentPoemas = poemasFavoritos.slice(
    startIndex,
  ...
  return (
    <>
      {currentPoemas.reverse().map(renderPoemas)}
      <div>
        ...
        <button
          ...
          disabled={startIndex + itemsPerPage >= poemasFavoritos.length}
        >
          Siguiente
        </button>
      </div>
